...
Consumer surplus is the gains from trade for consumers (and likewise, producer surplus is gains from trade for sellers) from that particular market transaction. 

If demand is quite price elastic, then the consumer is very sensitive to price changes of this good (due to, e.g. many substitutes available, lots of time to adjust, etc.) and is not willing to pay all that much more than the market price. Consumers get quite a small amount of benefit from this particular transaction. In terms of the math/geometry, this means that the height of the triangle (the difference between the choke price and market price) is relatively small for the same base (equilibrium quantity) as the demand curve slope is quite flat. 

If demand is quite price inelastic, then the consumer is not very sensitive to price changes of this good (due to, e.g. few substitutes available, this good being a necessity, etc.) and is willing to pay a lot more than the market price. Consumers get quite a large amount of benefit from this particular transaction. In terms of the math/geometry, this means that the height of the triangle (the difference between the choke price and market price) is relatively large for the same base (equilibrium quantity) as the demand curve slope is quite steep.
...
inelastic<-ggplot(data.frame(x=c(0,10)), aes(x=x))+
  ...
  stat_function(fun=supply, geom="line", size=2, color = "red")+
    geom_label(aes(x=9,y=supply(9)), color = "red", label="Supply (Min WTA)", size = 3)+
  geom_polygon(data=cs_1, aes(x=x,y=y), fill="blue", alpha=0.5)+
  geom_segment(x=0, xend=5, y=5, yend=5, size=1, linetype="dotted")+
  geom_segment(x=5, xend=5, y=0, yend=5, size=1, linetype="dotted")+
  ...
  labs(x = "Quantity (q)",
       y = "Price (p)",
       title = "Relatively Inelastic Demand")+
  theme_classic(base_family = "Fira Sans Condensed", base_size=14)
...
elastic<-ggplot(data.frame(x=c(0,10)), aes(x=x))+
  stat_function(fun=demand_el, geom="line", size=2, color = "blue")+
    geom_label(aes(x=9,y=demand_el(9)), color = "blue", label="Demand (Max WTP)", size = 3)+
  stat_function(fun=supply, geom="line", size=2, color = "red")+
    geom_label(aes(x=9,y=supply(9)), color = "red", label="Supply (Min WTA)", size = 3)+
  geom_polygon(data=cs_el, aes(x=x,y=y), fill="blue", alpha=0.5)+
  geom_segment(x=0, xend=5, y=5, yend=5, size=1, linetype="dotted")+
  geom_segment(x=5, xend=5, y=0, yend=5, size=1, linetype="dotted")+
    scale_x_continuous(breaks=seq(0,10,1),
                     limits=c(0,10),
  ...
  labs(x = "Quantity (q)",
       y = "Price (p)",
       title = "Relatively Elastic Demand")+
  theme_classic(base_family = "Fira Sans Condensed", base_size=14)
...
ggplot(data.frame(x=c(0,10)), aes(x=x))+
  stat_function(fun=demand_1, geom="line", size=2, color = "blue")+
    geom_label(aes(x=9,y=demand_1(9)), color = "blue", label="MSB", size = 4)+
  stat_function(fun=supply_1, geom="line", size=2, color = "#e64173")+
    geom_label(aes(x=9,y=supply_1(9)), color = "#e64173", label="MSC", size = 4)+
  geom_segment(x=0, xend=5, y=5, yend=5, size=1, linetype="dotted")+
  geom_segment(x=5, xend=5, y=0, yend=5, size=1, linetype="dotted")+
    scale_x_continuous(breaks=seq(0,10,1),
                     limits=c(0,10),
  ...
  stat_function(fun=supply_up, geom="line", size=2, color = "#fb6107")+
    geom_label(aes(x=9,y=supply_up(9)), color = "#fb6107", label="MPC", size = 4)+
  ...
  geom_label(x=8, y=5, label="External Cost", color = "purple", size =4)+
...
Someone commits the nirvana fallacy when they compare an system in reality with all of its imperfections and failures with an idealized perfect system. The alternative to one flawed system is not another system that is perfect, with omniscient and omnibenevolent actors, the realistic alternative is another flawed system, populated by the same people (with all of their flaws) facing different institutions and incentives.

One common example is observing a market failure (say, perhaps an externality or underprovision of a public good) and proposing an ideal government solution — one that requires a government that works perfectly well, with no knowledge or incentive problems. In fact, government might fail *worse* than markets in addressing the problem. 

Rather than *a priori* comparisons, we need to turn to empirical evidence to determine which system fails the *least* in the presence of knowledge and incentive problems.
